...
def new_user():
    ...
    login = driver.find_element(By.ID,'login').send_keys('Mimi')
    pass_inp = driver.find_element(By.ID,'password').send_keys('NewPass24')
...
def new_users():
    ...
    WebDriverWait(driver,5).until(EC.element_to_be_clickable((By.LINK_TEXT, 'Вход'))).click() 
    name = ['Kili', 'Lili', 'Gigi']
    passw = ['NewPass241', 'NewPass242', 'NewPass243']
    counter = 3

    while counter>0:
        new_login = driver.find_element(By.ID,'newLogin').send_keys(random.choice(name))
        new_pass = driver.find_element(By.ID,'newPassword').send_keys(random.choice(passw))
        WebDriverWait(driver,5).until(EC.element_to_be_clickable((By.XPATH, "//button [text()='Добавить']"))).click()
        messagen = driver.find_element(By.ID,'addUserMessage').text
        assert 'Пользователь добавлен!' == messagen
        counter -= 1

    text1 = str(driver.find_element(By.ID,'userTable').text)
    print(text1)   
    print(text1.index(12))
    print(text1.index(13))

    #driver.find_element(By.ID,'login').send_keys(text1.index[12])
   #driver.find_element(By.ID,'password').send_keys(text1.index[13])
